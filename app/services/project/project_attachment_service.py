...
class ProjectAttachmentService:
    def __init__(self, db: Session):
        ...
        self.static_folder_image = "static/images/project_attachment/attachment"

    def create_project_attachment(self, project_attachment: ProjectAttachment, image: UploadFile = None, file_extension_image = None):
        project_attachment.image_url = upload_file(image, self.static_folder_image, file_extension_image,  f"{project_attachment.user.username}-{project_attachment.title}") if image else ''
    ...
    def update_project_attachment(
        ...
        if image and not exist_project_attachment.image_url:
            exist_project_attachment.image_url = upload_file(image, self.static_folder_image, file_extension_image,  f"{project_attachment.user.username}-{project_attachment.title}")
